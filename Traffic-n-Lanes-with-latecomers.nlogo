globals [
  ...
  seed           ; the seed value for the random number generator [Fabrício]
]
...
turtles-own [
  ...
  original-top-speed  ; stores the original maximum speed of the car ; [Fabrício]
]

patches-own [ ;new proprieties for patches [Fabrício]
  original-color ; Store the original color of the patch [Fabrício]
  lifetime       ; remaining lifetime of the yellow patch [Fabrício]
  max-speed      ; maximum speed allowed on this patch ; [Fabrício]
]

to setup
  clear-all
  set-default-shape turtles "car"
  if seed-input != "" [                    ; checks if the user chose to use a seed, or use a random seed [Fabrício]
    set seed (read-from-string seed-input) ; [Farício]
    random-seed seed                       ; [Fabrício]
  ]
  draw-road
...
to create-or-remove-cars
  ...
  create-turtles (number-of-cars - count turtles) [
    ...
    set original-top-speed top-speed  ; store the original maximum speed ; [Fabrício]
    set speed 0.5
...
to draw-road
  ask patches [
    set pcolor green - random-float 0.5  ; Define a grass color as green with hue variation
  ]

  ; Define road lanes based on the number of lanes
  set lanes n-values number-of-lanes [ n -> number-of-lanes - (n * 2) - 1 ]

  let road-patches patches with [ abs pycor <= number-of-lanes ]  ; Select patches composing the road

  ask road-patches [
    set pcolor grey - 2.5 + random-float 0.25 ; Define road color as grey with shade variation
    set original-color pcolor                ; Store original patch color (grey)
    set max-speed 1                          ; Define maximum speed for normal patches as 1 ; [Fabrício]
  ]

  ; Randomly select road patches to become yellow [Fabrício]
  ask n-of (alert-percent / 100 * count road-patches) road-patches [
    set pcolor yellow                                     ; Define patch color as yellow [Fabrício]
    set lifetime random (number-of-ticks - 1) + 1         ; Define random lifetime for yellow patch (between 1 and maximum ticks) [Fabrício]
    set max-speed 0.01 + random-float 0.39                ; Define random maximum speed for yellow patches between 0.01 and 0.40 ; [Fabrício]
  ]

  draw-road-lines ; Draw road lines (white and yellow)
end
  ...
  while [ y <= first lanes + 1 ] [
    if not member? y lanes [
      ...
      ifelse abs y = number-of-lanes
        [ draw-line y yellow 0 ]  ; yellow for the sides of the road [Fabrício]
...
to go
  ...
  ; Update yellow patches [Fabrício]
  ask patches with [
    original-color != yellow  ; Original color is not yellow (i.e., it's grey) [Fabrício]
    and pcolor = yellow       ; Current color is yellow [Fabrício]
    and lifetime > 0          ; Still has lifetime remaining [Fabrício]
  ] [
    set lifetime lifetime - 1 ; [Fabrício]
    if lifetime = 0 [         ; [Fabrício]
      set pcolor original-color ; Restore original color (grey) [Fabrício]
    ]
  ]

  if ticks > number-of-ticks - 1 [stop] ; Added an input box to select the maximum number of ticks [Fabrício]
...
to move-forward ; turtle procedure
  ...
  ; Adjust the speed based on the patch's max-speed ; [Fabrício]
  if speed > [ max-speed ] of patch-here [ ; [Fabrício]
    set speed [ max-speed ] of patch-here ; [Fabrício]
  ] ; [Fabrício]
  ...
  ; Adjust the speed again after considering blocking cars ; [Fabrício]
  if speed > [ max-speed ] of patch-here [ ; [Fabrício]
    set speed [ max-speed ] of patch-here ; [Fabrício]
  ] ; [Fabrício]
  ...
  ; If on a normal patch, restore the car's original top speed ; [Fabrício]
  if [ max-speed ] of patch-here = 1 [ ; [Fabrício]
    set top-speed original-top-speed ; [Fabrício]
  ] ; [Fabrício]
end
...
to slow-down-car ; turtle procedure
  ...
  ; every time you hit the brakes, you lose a little patience
...
; The changes were made to the lines with comments like [Fabrício];
@#$#@#$#@
GRAPHICS-WINDOW
225
70
